...
AXUIElementRef _menuItemForTitle(AXUIElementRef menuBarRef, NSString *targetTitle)
{
	...
	NSInteger menuCount = [menuItemArray count]; // NSIntegerに変更
	
	AXUIElementRef menuItem;
	id menuTitle;
	NSInteger i; // NSIntegerに変更
	...
	for (i = 0; i < menuCount; i++) {
		...
		if (err == kAXErrorSuccess) {
			...
			if ([menuTitle isEqual:targetTitle]) return menuItem;
		}

		// Dig Into Child Nodes
		AXUIElementRef foundItem = _menuItemForTitle(menuItem, targetTitle);
		if (foundItem) {
			return foundItem;
		}
	}
...
{
	AXError err;
	AXUIElementRef targetMenuItem = NULL;
	id targetMenuTitle;
	...
	if (err != kAXErrorSuccess) {
		if (err == kAXErrorAPIDisabled) {
			...
			NSLog(@"Could not obtain AXUIElement for application menu bar. err=%d", (int)err);
		}
...
- (BOOL)_isModifiersPressed
{
	...
	k = (long)CFSwapInt32BigToHost(km[1].bigEndianValue);
	#else
	k = (long)km[1];
	#endif
...
- _generatePseudoEvent:(int)eventId
{
	...
	if ([self _isModifiersPressed]) {
		NSLog(@"A gesture has been blocked because some modifiers are pressed.");
		return self;
	}
	
	NSString *bundlePath = self.activeApplication.bundleURL.path;
	appHotkeyDefs = [eventKeyDefs objectForKey:bundlePath];
	
	if (appHotkeyDefs && (NSUInteger)eventId < [appHotkeyDefs count]) {
		thisKey = [appHotkeyDefs objectAtIndex:(NSUInteger)eventId];
		if ([thisKey pseudoEventType] == HotkeyInherit) {
			thisKey = [keyDefsGlobal objectAtIndex:(NSUInteger)eventId];
		}
	} else {
		thisKey = [keyDefsGlobal objectAtIndex:(NSUInteger)eventId];
	}
	...
	case HotkeyMenuItem:
		...
		break;
	default:
		NSLog(@"Unknown pseudo event type %d", (int)[thisKey pseudoEventType]);
	}
	return self;
}
...
- _composeGesture:(EGMoveDirection)thisMove;
{
	...
	thisTimestamp = (unsigned long)(tv.tv_sec * 1000 + (tv.tv_usec / 1000));
...
- init
{
	...
	gestureSizeMin = (int)[defaults integerForKey:@"GestureSizeMin"];
